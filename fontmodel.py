...
class MultiheadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        a_t = torch.matmul(q.squeeze(), k.squeeze().swapaxes(-2, -1)) * self.scale # (N, num_heads, 1 + seq_len, 1 + seq_len)
        a_t = a_t.softmax(dim=-1)
        if self.masked:
            a_t = torch.tril(a_t) # (N, num_heads, 1 + seq_len, 1 + seq_len)
            a_t = a_t / torch.sum(a_t, dim=-1, keepdim=True)
        attn_vals = torch.matmul(a_t, v.squeeze()).swapaxes(-3, -2).flatten(-2, -1) # concatenates heads; (N, 1 + seq_len, num_heads * head_size) = (N, 1 + seq_len, embedding_dim)
...
class FontModel(nn.Module):
    def __init__(self, num_layers : int, vocab_size : int, embedding_dim : int, num_heads : int, ff_dim : int):
